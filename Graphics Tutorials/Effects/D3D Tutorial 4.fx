...
VS_OUT VS(VS_IN vIn)
{
	...
	output.TangentW = mul(float4(vIn.TangentL, 0.0f), World);
	output.Normal = mul(float4(vIn.NormalL, 0.0f), World);
	//output.Normal = normalize(output.Normal);

	output.Pos = mul( float4(vIn.PosL,1.0f), mul(mul(World,View),Projection));
    //output.Pos = mul( float4(output.Pos,1.0f), View );
    //output.Pos = mul( float4(output.Pos,1.0f), Projection );
...
float4 PS(VS_OUT pIn) : SV_Target
{
	...
	float3 norm = gTextureBump.Sample(gBumpSampler,pIn.TexC);
	...
	float3 bumpedNormalW = normalize (mul(norm,TBN));

	SurfaceInfo v = {pIn.PosW,bumpedNormalW,diffMtrl,specMtrl};
	...
	// Compute Colour
	return float4(litColor, diffMtrl.a);
}
